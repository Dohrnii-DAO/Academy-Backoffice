...
import "../../../Assets/Css/editlessons.css";
...
const UpdateLessons = ({ location }) => {
  ...
  const UpdateData = async (event) => {
    ...
    const LessonData = {
      ...
      classes: classdata.map(({ classes: { id: classId } }, id) => ({
        class_id: classId,
        sequence: id + 1,
      })),
    };
    ...
    // return false;
    setIsloading(true);
    let LessonAdd = await dispatch(updateLesson(LessonData));
    console.log(LessonAdd);
    if (LessonAdd?.statuscode === 200) {
  ...
  const addClass = async (e) => {
    ...
    if (
      ...
      classdata.map((element) => element.classes.title).indexOf(className)
    ) {
      ...
      setClassdata((preState) => [
        ...preState,
        { classes: { id: classId, title: className } },
      ]);
    }
    ...
    const LessonData1 = {
      ...
      classes: [
        {
          class_id: classId,
          sequence: Number(classdata.length) + 1,
        },
      ],
    };
    setShowHide(!showHide);
    setIsloading(true);
    await dispatch(updateLesson(LessonData1));
    setIsloading(false);
    let data = await LessonClassGet(id);
    setClassdata(data.data.data);
    setClassNumber("");
  };
  ...
  const ConfirmDelete = async () => {
    ...
    try {
      setIsloading(true);
      await ClassLessondelete(deletReletion, deleteClassId);
      let data = await LessonClassGet(id);
      setClassdata(data?.data?.data);
      ...
      if (e.response?.data?.data === undefined) {
        ...
        return setClassdata(e?.response?.data?.data);
      }
    }
  };
  ...
  return (
    <>
      <Container fluid className="edit-lessons-section">
        {/* Start edit Lessons card */}
        {/* Start edit Lessons card header */}
        <Row className="edit-lessons">
          <Col className="edit-lessons-title">
            <Row className="ps-lg-3 ps-md-3">
              ...
              <Col sm="6" md="12" lg="12">
                <h6>
                  <Breadcrumbs showUL={true} showL={true} />
                </h6>
              </Col>
            </Row>
          </Col>
        ...
        <BlockUi tag="div" blocking={isloading}>
          {/* End edit Lessons card header */}
          {/* Start edit Lessons */}
          <Row>
            {/* Start left edit Lessons form */}
            <Col md="6" sm="12" lg="6">
            ...
            {/* End left edit Lessons form */}
            {/* Start Right edit Lessons form */}
            <Col md="6" sm="12" lg="6">
                  ...
                  <Form className="right-chapter-form lesson-classes">
                    ...
                    <div className="Mainlist">
                      ...
                      <ReactDragListView {...dragProps}>
                        {classdata?.length
                          ? classdata.map(
                              ({ classes: { title, sequence, id } }, index) => (
